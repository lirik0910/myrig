                    ...
                    <div class="article-row row">
                        <div class="col-sm-4">
                                ...
                                @foreach($product->options as $option)
                                    @if($option->name == 'image')
                                        <div class="product-item @if($loop->first) active @endif">
                                            <img width="300" height="300" src="{{asset($option->value)}}" class="attachment-medium size-medium" alt="" title="" data-src="{{asset($option->value)}}" data-large_image="{{asset($option->value)}}" data-large_image_width="1280" data-large_image_height="1280" sizes="(max-width: 300px) 100vw, 300px" />
                                        </div>
                                    @endif
                                @endforeach
                            </div>
                            <div class="itemSliderVer visible-md">
                                @foreach($product->options as $option)
                                    @if($option->name == 'image')
                                        <div class="product-item @if($loop->first) active @endif">
                                            <img width="47" height="47" src="{{asset($option->value)}}" class="attachment-i47 size-i47" alt="" title="" data-src="{{asset($option->value)}}" data-large_image="{{asset($option->value)}}" data-large_image_width="1280" data-large_image_height="1280" sizes="(max-width: 47px) 100vw, 47px" />
                                        </div>
                                    @endif
                                @endforeach
                            </div>
                        ...
                        <div class="col-sm-8">
                            <div class="article-text">
                                <h2><a href="{{url('shop/' . $product->id .'')}}" data-wpel-link="internal">{{$product->title}}</a></h2>
                                <div class="tag tag-order">Предзаказ</div>

                                <div class="tag tag-waranty" >@foreach($product->options as $option)@if($option->name == 'warranty'){{$option->value}} @endif @endforeach</div>
                                <div><ul>
                                        @foreach($product->options as $option)
                                            @if($option->name == 'previewInfo')
                                                @php
                                                    $previewArr = explode('!', $option->value);
                                                @endphp
                                                @foreach($previewArr as $str)
                                                    <li>{{$str}}</li>
                                                @endforeach
                                           @endif
                                        @endforeach
                                ...
                                <div class="single-product-price">
                                    <span class="woocommerce-Price-amount amount"><span class="woocommerce-Price-currencySymbol">&#36;</span>{{$product->price . '.00'}}</span>					            <span class="table-bitcoin">0.2433<i class="fa fa-bitcoin"></i></span></div>
